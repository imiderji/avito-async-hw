...
async def coroutines_execution_order(coros: list[Awaitable[Ticket]]) -> str:
    # Необходимо выполнить все полученные корутины, затем упорядочить
    # их результаты по полю number и вернуть строку
    ...
    coroutines_results = await asyncio.gather(*coros)
